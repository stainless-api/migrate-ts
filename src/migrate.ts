...
export default function createTransformer({
  ...
  return function transformer(file: FileInfo) {
    ...
    function importName(name: string, from: string, asName?: string) {
      for (const [, [specifier, source]] of importBindings) {
        if (source.replace(/^node:/, "") !== from.replace(/^node:/, ""))
          continue;
        if (
      ...
      CallExpression(path) {
        ...
        if (pattern.type === "Identifier") {
          ...
          pattern.properties.forEach((prop, i) => {
            ...
            if (
              regexp`^${pkg}(\/(index|uploads)(\.m?js)?)?$`.test(
                importSource
              ) &&
              ...
              if (fetchProp) {
                ...
                ref.parent.arguments[0].properties.push(
                  t.objectProperty(
                    t.identifier("fetch"),
                    t.identifier(
                      importName("default", "node-fetch", "nodeFetch")
                    )
                  )
                );
              }
    ...
    traverse.default(parsed, {
      ReferencedIdentifier(path) {
        if (clientBindings.has(lookupBinding(path.scope, path.node.name))) {
          let keys: string[] = [];
          ...
          if (keys.length) {
            ...
            if (method) {
              ...
              if (v1Params && parentNode.type === "CallExpression") {
                let args = parentNode.arguments.filter(
                  (e): e is t.Expression => {
                    if (
                ...
                const lastDefined = findLastIndex(
                  args,
                  (e) =>
                    e && !(e.type === "Identifier" && e.name === "undefined")
                );
